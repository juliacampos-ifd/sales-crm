    ...
    return FUNNEL_ROWS.map(def => {
      ...
      monthCols.forEach(col => {
        ...
        if (def.key === 'elegiveis') {
          if (isCur) { row.cells.push({ isCur:true, meta:elegMeta, fcst:eleg, pctA:elegMeta>0?Math.round((eleg/elegMeta)*100)+'%':'—', real:eleg, mtdMeta:eleg, mtdReal:eleg, mtdPct:'100%' }); }
          else { row.cells.push({ v: gm(dupla, col.y, col.m, 'elegiveis') }); }
          ...
          if (isCur) { const metaML = gm(dupla, selYear, selMonth, 'media_lojas'); row.cells.push({ isCur:true, meta:metaML, fcst:v, pctA:metaML>0?Math.round((v/metaML)*100)+'%':'—', real:v, mtdMeta:v, mtdReal:v, mtdPct:'—', isLive:true }); }
          else { row.cells.push({ v }); }
          ...
          } else { row.cells.push({ v: gr(dupla, col.k, f) }); }
        }
      });
      return row;
    });
                      ...
                      {rows.map((row,ri) => (
                        <tr key={ri} style={{ background:row.isBold?'#fffbfb':'#fff' }}>
                          ...
                          {row.cells.map((cell,ci) => {
                            if (!cell.isCur) return <td key={ci} style={{ ...td, textAlign:'center', fontWeight:row.isBold?600:400, color:row.isPercent?'#94a3b8':'#475569' }}>{cell.v}</td>;
