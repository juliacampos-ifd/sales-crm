...
const DUPLA_COLORS = {
  ...
  lidia_gabi: '#ec4899',
  joao_diego: '#3b82f6',
  michel_emerson: '#22c55e',
};

const FUNNEL_ROWS = [
  { key: 'elegiveis', label: 'MARCAS ELEGIVEIS', isBold: true, noMeta: true },
  ...
  { key: 'media_lojas', label: 'Media de lojas por marca', noMeta: true },
...
export default function ScorecardPage() {
    ...
    return FUNNEL_ROWS.map(def => {
      ...
      monthCols.forEach(col => {
        ...
        if (def.key === 'elegiveis') {
          const v = isCur ? eleg : gm(dupla, col.y, col.m, 'elegiveis');
          row.cells.push(isCur ? { v, meta: gm(dupla, col.y, col.m, 'elegiveis'), isCur: true, span: true } : { v });
          ...
          row.cells.push(isCur ? { v, meta: gm(dupla, col.y, col.m, 'media_lojas'), isCur: true, span: true } : { v });
        } else if (def.isPercent) {
          let num = 0, den = 1;
          const getVal = (field) => isCur ? cmR[field] : gr(dupla, col.k, field);
          if (def.key === 'taxa_pc') { num = getVal('primeiro_contato'); den = isCur ? eleg : gm(dupla, col.y, col.m, 'elegiveis'); }
          else if (def.key === 'taxa_apres') { num = getVal('apresentacao'); den = getVal('primeiro_contato'); }
          else if (def.key === 'taxa_neg') { num = getVal('negociacao'); den = getVal('apresentacao'); }
          else if (def.key === 'taxa_fechadas') { num = getVal('fechadas'); den = getVal('negociacao'); }
          const pct = den > 0 ? Math.round((num / den) * 100) + '%' : '0%';
          row.cells.push(isCur ? { v: pct, isCur: true, span: true } : { v: pct });
        } else {
          const f = def.key;
          if (isCur) {
  ...
  if (!user) return <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><p>Faca login primeiro</p><a href="/" style={{ color: '#6366f1' }}>Login</a></div>;
      ...
      <div style={{ background: '#fff', borderBottom: '1px solid #e2e8f0', padding: '10px 28px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 40 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <div style={{ width: 32, height: 32, borderRadius: 8, background: 'linear-gradient(135deg, #6366f1, #8b5cf6)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><TrendingUp size={18} color="#fff" /></div>
            <span style={{ fontSize: 18, fontWeight: 800, color: '#6366f1' }}>Scorecard</span>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div style={{ background: '#f1f5f9', borderRadius: 8, padding: '6px 12px', fontSize: 12, color: '#64748b' }}>{mtdBD}/{totalBD} dias uteis</div>
        </div>
      </div>
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 900 }}>
                    <thead>
                      <tr style={{ background: '#f8fafc' }}>
                        ...
                        {hasCur && <>
                          <th style={{ ...th, background: '#f0fdf4', fontSize: 10 }}>{MONTH_NAMES[selMonth-1]} Meta</th>
                          <th style={{ ...th, background: '#f0fdf4', fontSize: 10 }}>Fcst</th>
                          <th style={{ ...th, background: '#f0fdf4', fontSize: 10 }}>% Atig</th>
                          <th style={{ ...th, background: '#dcfce7', fontSize: 10 }}>Real</th>
                    ...
                    <tbody>
                      {rows.map((row, ri) => (
                        <tr key={ri} style={{ background: row.isBold ? '#fafbff' : '#fff' }}>
                          <td style={{ ...td, fontWeight: row.isBold ? 700 : 400, fontSize: row.isPercent ? 11 : 12, color: row.isPercent ? '#94a3b8' : '#1e293b', position: 'sticky', left: 0, background: row.isBold ? '#fafbff' : '#fff', zIndex: 1 }}>{row.label}</td>
                          {row.cells.map((cell, ci) => {
                            if (!cell.isCur) {
                              return <td key={ci} style={{ ...td, textAlign: 'center', fontWeight: row.isBold ? 600 : 400, color: row.isPercent ? '#94a3b8' : '#475569' }}>{cell.v}</td>;
                            }
                            if (cell.span) {
                              return <td key={ci} colSpan={7} style={{ ...td, textAlign: 'center', fontWeight: row.isBold ? 600 : 400, color: row.isPercent ? '#94a3b8' : '#475569' }}>{cell.v}</td>;
                            }
                            return [
                              <td key={ci+'m'} style={{ ...td, textAlign: 'center', background: '#f0fdf408' }}>{cell.meta}</td>,
                              <td key={ci+'f'} style={{ ...td, textAlign: 'center', fontWeight: 600, background: '#f0fdf408' }}>{cell.fcst}</td>,
                              <td key={ci+'p'} style={{ ...td, textAlign: 'center', fontWeight: 600, color: pctColor(cell.pctA), background: '#f0fdf408' }}>{cell.pctA}</td>,
                              <td key={ci+'r'} style={{ ...td, textAlign: 'center', fontWeight: 700, color: clr, background: '#dcfce708' }}>{cell.real}</td>,
